...
for l in inFile:
	...
	try:
		...
		pass
	if found==1:
		if int(line[11])==lookup:
			no=no+1

			if no%100==1:
				if no==1:
					outFile= codecs.open("layer1.csv", "w", encoding="utf-8", errors="replace")
					outFile.write("name,address,latitude,longitude,profession\n")
				else:
					outFile.close()
					newname="layer"+str(no/100)+".csv"
					outFile= codecs.open(newname, "w", encoding="utf-8", errors="replace")
					outFile.write("name,address,latitude,longitude,profession\n")
			print kill_accents(line[1])
