...
outFile= codecs.open("job"+lupsstr+".csv", "w", encoding="utf-8", errors="replace")
...
for l in inFile:
	...
	try:
		...
		pass
	if found==1 and int(line[11])<940:
		if lupsstr in japanize(line[-2]):
			...
			try:
				...
				#outline=line[1]+","+kill_accents(line[12])+" "+line[13]+","+str(0)+","+str(0)+","+fogl	+"\n"
